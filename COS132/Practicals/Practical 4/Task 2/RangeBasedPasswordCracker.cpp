...
int decideAndSearch(int guess, int password, int low, int high) {
    isLessThan(guess, password) ? crackPassword(guess + 1, high, password) : isGreaterThan(guess, password) ? crackPassword(low, guess - 1, password) : return password;
}
...
int crackPassword(int low, int high, int password) {
    ...
    printGuess(midpoint);
    isEqual(password, guess) ? foundPassword() : decideAndSearch();    
}
